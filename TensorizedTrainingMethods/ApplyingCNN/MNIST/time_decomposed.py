...
mysetup = '''
from PackagesAndModels.pack import weight_reset
from PackagesAndModels.train_val_test_CIFAR10 import load_cifar, evaluate_cifar
...
from PackagesAndModels.CIFAR_MODELS import convNet500_3d, convNet500
...
for c in convName:
  ...
  for cc in convData:
      t = cc.cpu()
      de_layer.append(parafac(tltensor(t), rank = 1)[1])
...
for l,layer in enumerate(pqt_convs):
  ...
  for f,fil in enumerate(layer):
    temp1[f] = torch.tensor(transpose(fil[0])).reshape(temp1.shape[1],temp1.shape[2],temp1.shape[3])
    temp2[f] = torch.tensor(transpose(fil[1])).reshape(temp2.shape[1],temp2.shape[2],temp2.shape[3])
...
mycode = '''
evaluate_cifar(testloader, net3D)
'''
...
from sklearn.metrics import accuracy_score
import torchvision.transforms as transforms
from PackagesAndModels.CIFAR_MODELS import convNet500_4D, convNet500
from numpy import transpose
import pickle
...
for c in convName:
  ...
  t = convData.cpu()
  pqtu_convs.append(parafac(tltensor(t), rank = 1)[1])
...
for l,layer in enumerate(pqtu_convs):
  ...
  temp1 = torch.tensor(transpose(layer[1])).reshape(temp1.shape[0],temp1.shape[1],temp1.shape[2],temp1.shape[3])
  temp2 = torch.tensor(transpose(layer[2])).reshape(temp2.shape[0],temp2.shape[1],temp2.shape[2],temp2.shape[3])
  temp3 = torch.tensor(transpose(layer[3])).reshape(temp3.shape[0],temp3.shape[1],temp3.shape[2],temp3.shape[3])
...
mycode = '''
evaluate_cifar(testloader, net4D)
'''
...
from sklearn.metrics import accuracy_score
import torchvision.transforms as transforms
from PackagesAndModels.CIFAR_MODELS import convNet500_4D_rank8, convNet500
from numpy import transpose
import pickle
...
#  pqtu_convs.append(parafac(tltensor(convData), rank = 8)[1])
...
mycode = '''
evaluate_cifar(testloader, net4D_r8)
'''
...
from sklearn.metrics import accuracy_score
import torchvision.transforms as transforms
from PackagesAndModels.CIFAR_MODELS import convNet500_Tucker211, convNet500
from numpy import transpose
import pickle
...
for l,layer in enumerate(utc_convs):
  ...
  temp1 = torch.tensor(transpose(layer[1])).reshape(temp1.shape[0],temp1.shape[1],temp1.shape[2],temp1.shape[3])
  temp2 = torch.tensor(transpose(layer[2], (1,0,2,3))).reshape(temp2.shape[0],temp2.shape[1],temp2.shape[2],temp2.shape[3])
...
mycode = '''
evaluate_cifar(testloader, netTucker2)
'''
...
from sklearn.metrics import accuracy_score
import torchvision.transforms as transforms
from PackagesAndModels.CIFAR_MODELS import convNet500_Tucker288, convNet500
from numpy import transpose
import pickle
...
mycode = '''
evaluate_cifar(testloader, netTucker2_r88)
'''
