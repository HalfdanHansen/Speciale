...
class BasicBlock4D(nn.Module):
    expansion = 1
    def __init__(self, in_planes, planes, stride = 1, rank = 1):
        skip = stride != 1 or in_planes != self.expansion*planes

        super(BasicBlock4D, self).__init__()
        self.conv1 = conv_4D_block(in_planes,planes,3,1+skip, rank=rank)
        self.bn1 = nn.BatchNorm2d(planes, affine=False, track_running_stats=False)
        self.conv2 = conv_4D_block(planes,planes,3,1, rank=rank)
        ...
        if skip:
            self.shortcut = nn.Sequential(
                conv_4D_block(in_planes,planes,1,1+skip,0, rank = 1),
...
class ResNet4D(nn.Module):
    def __init__(self, block, num_blocks, rank = 1, num_classes=10):
        ...
        self.conv1 = conv_4D_block(3,64,3,1, rank=rank)
    ...
    def _make_layer(self, block, planes, num_blocks, stride, rank = 1):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride=stride, rank=rank))
...
for M in range(NumModels):
    for repeats in range(5):
        if M < 4:
            net = ResNet4D(BasicBlock4D, [M+1,M+1,M+1,M+1], 1)
        elif M < 8:
            net = ResNet4D(BasicBlock4D, [M-3,M-3,M-3,M-3], 2)
        else:
            net = ResNet4D(BasicBlock4D, [M-7,M-7,M-7,M-7], 3)
        net.cuda()
...
pd.concat([save_train,save_test,save_loss],axis = 0).to_csv('firstTry.csv',index=False,header=False)
